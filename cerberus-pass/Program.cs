...
while (masterPasswordSet == false)
{
  ...
  {
    userInterface.EmptyStringError();
    Console.ReadLine();
    continue;
  }
  userInterface.PromptConfirmPassword();
  if (!manager.CheckPasswordConfirmation(newMasterPassword, userInterface.GetParameter("das Password erneut")))
  {
    userInterface.WrongPasswordError();
    Console.ReadLine();
    continue;
  }
...
do
{
  ...
  switch ((MenuOptions)menuChoice)
  {
    ...
    case MenuOptions.DisplayOneEntry:
      if (!manager.CheckMasterPassword(userInterface.GetParameter("das Master-Passwort")))
      {
        userInterface.WrongPasswordError();
        break;
      }
      if (manager.GetVault().Count == 0)
      {
        userInterface.NoEntriesError();
        break;
      }
    ...
    case MenuOptions.CreateNewEntry:
      ...
      if (newTitle == "")
      {
        userInterface.TitleLengthError();
        break;
      }
      else if (manager.TitleExists(newTitle))
      {
      ...
      userInterface.PromptConfirmPassword();
      if (!manager.CheckPasswordConfirmation(newPassword, userInterface.GetParameter("das Password erneut")))
      {
        userInterface.WrongPasswordError();
        break;
      }

      // var encryptedPW = encrypter.EncryptPassword(newPassword);
    ...
    case MenuOptions.UpdateEntry:
      ...
      else if (!manager.TitleExists(titleToChange))
      {
        userInterface.TitleError("nicht");
        break;
      }
      ...
      userInterface.PromptConfirmPassword();
      if (!manager.CheckPasswordConfirmation(updatedPassword, userInterface.GetParameter("das Password erneut")))
      {
        userInterface.WrongPasswordError();
        break;
      }
    ...
    case MenuOptions.DeleteEntry:
      ...
      if (!manager.TitleExists(titleToDelete))
      {
        userInterface.TitleError("nicht");
        break;
      }
